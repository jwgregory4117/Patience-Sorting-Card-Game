...
class CardSorter():
    ...
    def play(self):
        ...
        The Algorithm:
         ...
         1.Construct the Card piles calling the
           construct_piles_subseqs method, which also places
           each dealt Card correctly on the SubSequence list.
         2.Display the Card Piles by calling the
           display_card_piles method
         3.Display the longest subsequences, calling
           the display_longest_subseq method
         4.Use the piles to make a sorted Deck,
           calling the make_sorted_deck method
    ...
    def construct_piles_subseqs(self):
           ...
           3. Inner Loop through each Pile starting from the 
              ...
              b. If there exists a Pile whose top Card is higher 
                 ...
                 iii. Add the card_from_deck to the list of 
                      ...
                      method
                 iv.  Add the card_from_deck onto the Pile: See Pile methods
                 v.   Break out of loop
           4. Check the flag:
        ...
        for i in range(self.CARDS_IN_DECK):
            ...
            placed_in_pile = False
            j = 0
            while j + 1 <= len(self.__piles):
                if self.__piles[j].get_top_card().compare(card_from_deck) == 1:
                    placed_in_pile = True
                    if j == 0:
                        prev_card = None
                    else:
                        prev_card = self.__piles[j - 1].get_top_card()

                    self.add_card_to_subseq(card_from_deck, prev_card)
                    self.__piles[j].add_card(card_from_deck)
                    break
                else:
                    j += 1

            if not placed_in_pile:
                new_pile = Pile()
    ...
    def add_card_to_subseq(self, current_card, prev_card):
        ...
        if prev_card_subseq is None:
            ...
            if first_card.compare(prev_card) == 0:
                ...
                for c in range(len(subseq_copy)):
    ...
    def find_subseq(self, prev_card):
        ...
        1. Check that the Card passed in is not None, If so, 
           return None.
        ...
        for i in range(len(self.__subseqs)):
            if self.__subseqs[i].contains_card(prev_card):
                return self.__subseqs[i]
            else:
                continue
    ...
    def display_card_piles(self):
        ...
        """
        if len(self.__piles) == 0:
            return "No piles to display."
        else:
            piles_to_str = ""
            for i in range(len(self.__piles)):
                piles_to_str += (str(self.__piles[i]) + '\n')
    ...
    def display_longest_subseq(self):
        ...
        for i in range(len(self.__subseqs)):
            if len(self.__subseqs[i]) == len(self.__piles):
                print(str(self.__subseqs[i]))
            else:
                continue
        ...
        for i in range(len(self.__subseqs)):
            if len(self.__subseqs[i]) == len(self.__piles):
                clone = self.__subseqs[i].clone_subseq()
                self.__subseqs[i].reverse_subseq()
                print(str(self.__subseqs[i]))
                self.__subseqs[i] = clone
            else:
                continue
           ...
           3. Inner Loop: Traverse each Pile using the range function
              a. If the Pile is not empty
                 ...
                 ii. Compare that first_card to the current low_card
                     and if the first_card is lower than or equal to the 
                     current low_card, then make the first_card the newest
                     low_card and the Pile index the current low_pile.
           4. Get the lowest card from the top of the pile at 
              index low_pile and add it to the Deck.               
        """
        
        for i in range(self.CARDS_IN_DECK):
            low_pile = 0
            low_card = Card(3, 13)
            for j in range(len(self.__piles)):
                if not self.__piles[j].is_empty():
                    first_card = self.__piles[j].get_top_card()
